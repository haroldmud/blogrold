...
function Header() {
  ...
  const handleSearched =()=>{
    (type.length === 0)?createNews(searching(false)):createNews(searching(true))
  }
  console.log(type)
  ...
  <header className={` ${isScroll? "py-4" : "py-1"} z-10 h-fit fixed w-full bg-white px-4  flex justify-between border-b`}>
        <div className="flex">
          ...
          <div className="my-auto ml-4">
            <form onSubmit={(e)=> {e.preventDefault(); handleSubmit()}} className="md:flex hidden bg-gray-100 rounded-3xl my-auto p-1 pr-4">
              <div className={`${isScroll ? "text-3xl" : "text-xl"} my-auto text-gray-400`}><CiSearch/></div>
              <input onChange={(event)=>{handleSearched();setType(event.target.value)}}  type="text" className=" bg-gray-100 outline-none pl-2 placeholder:text-gray-400" placeholder="Search Blogrold" />
            </form>
          </div>
        </div>
