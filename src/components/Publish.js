...
function Publish() {
  ...
  return (
    <section className={`border w-[20rem] h-[30rem] scroll overflow-y-scroll`}>
      <h3 className="font-bold text-blue-900 sticky top-0 bg-white w-[19.2rem] p-2 border-b">
        Publishers
      </h3>
        ...
        {based.map((item, idx) => (
          <div>
            ...
            {item === check && (
              <div
                className={`${
                  switched ? "fixed" : "hidden"
                } lg:h-[100vh] h-[100rem] pb-12  w-full left-0 z-30 color top-0`}
              >
                <div className="shadow lg:w-6/12 w-11/12 lg:h-[38rem] h-[50rem] mt-4 overflow-y-scroll scroll pb-4 bg-white  mx-auto">
                  <div className="flex justify-end bg-white  h-12">
                    <button
                      ...
                      className="p-4 fixed bg-white top-4"
                    >
                      <VscChromeClose />
                    </button>
                  </div>
